...
class Block(nn.Module):
    ...
    def forward(self, x, mask_ratio=0., mask_ratio_skip=0.):
        x = self.mask_tokens(x, mask_ratio_skip) + self.drop_path(self.attn(self.norm1(x), mask_ratio))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x

    def mask_tokens(self, x, mask_ratio):
        # Substract 1 because we do not want to mask class token
        mask_head = int((x.shape[1] - 1) * mask_ratio)
        diag = torch.cat((torch.zeros(mask_head), torch.ones((x.shape[1] - 1) - mask_head)))
        diag = diag[torch.randperm(diag.nelement())]
        diag = torch.cat((torch.ones(1), diag))
        diag = torch.diag(diag).to(x.device)
